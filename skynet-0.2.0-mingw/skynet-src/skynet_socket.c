...
{
	socket_server_start_thread(SOCKET_SERVER);
}

static int socket_callback(int type, struct socket_message* result);
...
skynet_socket_init() {
	...
	socket_server_set_callback(SOCKET_SERVER, socket_callback, socket_thread_init);
}
...
void wakeup_worker();
static int
socket_callback(int type, struct socket_message* result)
{
	switch (type)
	{
	case SOCKET_EXIT:
