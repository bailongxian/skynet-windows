...
//#define LOCK(q) EnterCriticalSection(&q->cs);
//#define UNLOCK(q) LeaveCriticalSection(&q->cs);
...
expand_queue(struct message_queue *q) {
	struct skynet_message *new_queue = (struct skynet_message*)skynet_malloc(sizeof(struct skynet_message) * q->cap * 2);
	int i;
	for (i=0;i<q->cap;i++) {
