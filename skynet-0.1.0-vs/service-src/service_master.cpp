...
MASTER_API struct master *
master_create() {
	struct master *m = (struct master*)skynet_malloc(sizeof(*m));
	int i;
...
_insert_name(struct master *m, char name[GLOBALNAME_LENGTH]) {
	...
	struct name * node = (struct name*)skynet_malloc(sizeof(*node));
...
_connect_to(struct master *m, int id) {
	...
	const char * portstr = strchr(ipaddress,':');
	if (portstr==NULL) {
