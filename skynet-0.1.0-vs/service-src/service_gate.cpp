...
GATE_API struct gate *
gate_create(void) {
	struct gate * g = (struct gate*)skynet_malloc(sizeof(*g));
	memset(g,0,sizeof(*g));
	g->listen_id = -1;
	return g;
}
...
_report(struct gate * g, const char * data, ...) {
	...
	va_start(ap, data);
	char tmp[1024];
	int n = vsnprintf_s(tmp, sizeof(tmp), data, ap);
	va_end(ap);
...
_forward(struct gate *g, struct connection * c, int size) {
	...
	if (c->agent) {
		...
		int n = _snprintf(tmp,32,"%d data ",c->id);
...
gate_init(struct gate *g , struct skynet_context * ctx, char * parm) {
	...
	//int n = sscanf_s(parm, "%c %s %s %d %d %d",&header,watchdog, binding,&client_tag , &max,&buffer);
	if (n<4) {
	...
	g->conn = (struct connection*)skynet_malloc(max * sizeof(struct connection));
