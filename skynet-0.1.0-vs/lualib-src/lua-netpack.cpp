...
#define LUA_NETPACK_API __declspec(dllexport)
...
struct queue {
	...
	struct netpack queue1[QUEUESIZE];
};
...
lclear(lua_State *L) {
	...
	for (i=q->head;i<q->tail;i++) {
		struct netpack *np = &q->queue1[i % q->cap];
...
push_data(lua_State *L, int fd, void *buffer, int size, int clone) {
	...
	struct netpack *np = &q->queue1[q->tail];
	if (++q->tail >= q->cap)
		q->tail -= q->cap;
	np->id = fd;
	np->buffer = buffer;
...
static int
lfilter(lua_State *L) {
	struct skynet_socket_message *message = (struct skynet_socket_message *)lua_touserdata(L,2);
...
lpop(lua_State *L) {
	...
	if (q == NULL || q->head == q->tail)
		return 0;
	struct netpack *np = &q->queue1[q->head];
	if (++q->head >= q->cap) {
		q->head = 0;
	}
...
lpack_string(lua_State *L) {
	...
	if (len <= SMALLSTRING) {
		...
		buffer = (uint8_t *)lua_newuserdata(L, len + 2);
	}
