...
#define LUA_SOCKET_API __declspec(dllexport)
...
lfreepool(lua_State *L) {
	struct buffer_node * pool = (struct buffer_node *)lua_touserdata(L, 1);
...
static int
lunpack(lua_State *L) {
	struct skynet_socket_message *message = (struct skynet_socket_message *)lua_touserdata(L,1);
...
lconnect(lua_State *L) {
	...
	struct skynet_context * ctx = (struct skynet_context *)lua_touserdata(L, lua_upvalueindex(1));
...
luaopen_socketdriver(lua_State *L) {
	...
	struct snlua *lua = (struct snlua *)lua_touserdata(L,-1);
