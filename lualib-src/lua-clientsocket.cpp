...
lconnect(lua_State *L) {
	...
	if (r == -1) {
		return luaL_error(L, "Connect %s %d %d failed", addr, port, WSAGetLastError());
	}
...
static int lstartup(lua_State *L)
{
	WSADATA wsaData;
...
/*		if (errno == EAGAIN || errno == EINTR) {
	...
	if (sz + r <= CACHE_SIZE) {
		...
		buffer = (char*)lua_newuserdata(L, r + sz);
		memcpy(buffer, last, sz);
...
static void usleep(DWORD waitTime){
...
#define LOCK(q) while (InterlockedCompareExchange(&(q)->lock,1, 1)) {}
#define UNLOCK(q) InterlockedExchange(&(q)->lock, 0);

struct queue {
	volatile long lock;
	int head;
	int tail;
	char * queue1[QUEUE_SIZE];
};
static unsigned __stdcall readline_stdin(void* arg)
{
/*static void *
...
//	pthread_create(&pid, NULL, readline_stdin, q);
	HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0, readline_stdin, NULL, 0, NULL);
