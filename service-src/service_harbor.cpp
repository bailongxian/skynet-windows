...
_new_queue() {
	struct msg_queue * queue = (struct msg_queue *)skynet_malloc(sizeof(*queue));
	queue->size = DEFAULT_QUEUE_SIZE;
	queue->head = 0;
	queue->tail = 0;
	queue->data = (struct msg *)skynet_malloc(DEFAULT_QUEUE_SIZE * sizeof(struct msg));
...
_hash_insert(struct hashmap * hash, const char name[GLOBALNAME_LENGTH]) {
	...
	struct keyvalue * node = (struct keyvalue *)skynet_malloc(sizeof(*node));
...
_hash_new() {
	struct hashmap * h = (struct hashmap *)skynet_malloc(sizeof(struct hashmap));
	memset(h,0,sizeof(*h));
	return h;
}
...
_connect_to(struct harbor *h, const char *ipaddress, bool blocking) {
	const char * port = strchr(ipaddress,':');
	if (port==NULL) {
		return -1;
	}
