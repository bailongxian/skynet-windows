...
_context_inc() {
	...
	InterlockedIncrement(&G_NODE.total);
}

static void
_context_dec() {
	//__sync_fetch_and_sub(&G_NODE.total,1);
	InterlockedDecrement(&G_NODE.total);
}
...
skynet_context_new(const char * name, const char *param) {
	...
	if (inst == NULL)
		return NULL;
	struct skynet_context * ctx = (struct skynet_context*)skynet_malloc(sizeof(*ctx));
...
skynet_context_release(struct skynet_context *ctx) {
	...
	if (InterlockedDecrement(&ctx->ref) == 0) {
		_delete_context(ctx);
		return NULL;
	}
	return ctx;
...
extern "C" SKYNET_API const char * 
skynet_command(struct skynet_context * context, const char * cmd , const char * param) {
	...
	if (strcmp(cmd,"REG") == 0) {
		if (param == NULL || param[0] == '\0') {
			...
			struct remote_name *rname = (struct remote_name*)skynet_malloc(sizeof(*rname));
	...
	if (strcmp(cmd,"NAME") == 0) {
		...
		char name[100];
		char handle[100];
		sscanf_s(param,"%s %s",name,handle);
		if (handle[0] != ':') {
			return NULL;
		}
...
_filter_args(struct skynet_context * context, int type, int *session, void ** data, size_t * sz) {
	...
	if (needcopy && *data) {
		char * msg = (char*)skynet_malloc(*sz+1);
		memcpy(msg, *data, *sz);
		msg[*sz] = '\0';
		*data = msg;
	}
...
skynet_send(struct skynet_context * context, uint32_t source, uint32_t destination , int type, int session, void * data, size_t sz) {
	...
	if (skynet_harbor_message_isremote(destination)) {
		struct remote_message * rmsg = (struct remote_message*)skynet_malloc(sizeof(*rmsg));
